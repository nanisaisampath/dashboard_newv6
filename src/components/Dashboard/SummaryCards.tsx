...
const SummaryCards = () => {
  ...
  const activeFiltersCardWidth = "200px"
  const activeFiltersCardHeight = "160px"

  // Calculate the number of active filters (not "All")
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-1">
              ...
              <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2">
                ...
                {filters.startDate && filters.endDate && (
                  <span className="text-2xl font-bold text-gray-500 dark:text-gray-400">
                    ({new Date(filters.startDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })} - {new Date(filters.endDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })})
                  </span>
                )}
              </h2>
        ...
        <div className="flex items-center gap-4">
          <Badge variant="outline" className="px-3 py-1 text-sm font-medium border-2">
      ...
      <div className="flex flex-wrap xl:flex-nowrap gap-4">
        ...
        {/* Active Filters Card */}
        <Card className="w-full border border-slate-700 bg-slate-900 shadow-lg rounded-xl overflow-hidden">
  <div className="bg-slate-800 px-5 py-3 border-b border-slate-700 flex items-center justify-between">
    <h3 className="text-base font-semibold text-white flex items-center gap-2">
      <Filter className="h-4 w-4" />
      Active Filters
    </h3>
    <span className="text-sm text-slate-400">{totalActiveFilters} Active Filters</span>
  </div>
  ...
  <CardContent className="px-4 py-3">
    {activeFilters.length === 0 ? (
      ...
      <div className="flex gap-4 overflow-x-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800 px-1 pb-2">
        {activeFilters.map((filter, index) => (
          <div
            key={index}
            className="shrink-0 bg-slate-800 border border-slate-700 rounded-lg p-3 min-w-[200px] shadow-sm"
          >
              ...
              {filter.selected.length > 0 ? (
                filter.selected.slice(0, 6).map((option, i) => (
                  <Badge
                    key={i}
                    variant="default"
                    className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-2 py-0.5 rounded-full"
                  >
                    {option}
                  </Badge>
                ))
                ...
                <Badge
                  variant="outline"
                  className="text-xs text-slate-400 border-slate-600 bg-slate-700 px-2 py-0.5 rounded-full"
                >
                  All
                </Badge>
              )}
...
const MetricCard = ({
  ...
  <Card
    style={{ width, height }}
    className={`group border-2 shadow-lg bg-gradient-to-br ${bgGradient} overflow-hidden ${
      clickable ? "hover:scale-105 hover:shadow-xl cursor-pointer" : ""
